...
def split_train_val():
    ...
    if not os.path.exists(os.path.join('./', 'val')):  # 如果資料夾不存在
        os.makedirs(os.path.join('./', 'val'))  # 建立新資料夾
...
def organize_pic_folder(path, val=False):
    ...
    if not os.path.exists(dst_folder):  # 如果資料夾不存在
        os.makedirs(dst_folder)  # 建立新資料夾
